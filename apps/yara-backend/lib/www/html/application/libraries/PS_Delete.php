...
class PS_Delete {
	...
	{	
		...
		@unlink('./uploads/thumbnail2x/'.$user_profile_photo);
		@unlink('./uploads/thumbnail3x/'.$user_profile_photo);
	...
	*/
	...
	/**
	 * Delete the customized header by api
	 *
	 * @param      <type>  $id     The identifier
	 */
	function delete_customized_header( $product_id )
	{		
		$conds = array( 'product_id' => $product_id );
		if ( ! $this->CI->Prd_attribute->delete_by( $conds )) {
		// if there is an error in deleting Product,
	...
	/**
	 * Delete the customized detail by api
	 *
	 * @param      <type>  $id     The identifier
	 */
	function delete_cusdetail( $header_id )
	{		
		$conds = array( 'header_id' => $header_id );
		if ( ! $this->CI->Attributedetail->delete_by( $conds )) {
		// if there is an error in deleting Product,
			return false;
		}
	 ...
	 * @param      <type>  $id     The identifier
	*/
	function delete_restaurant_branch( $restaurant_id, $enable_trigger = false )
	{		
		if ( ! $this->CI->Restaurant_branch->delete( $restaurant_id )) {
		// if there is an error in deleting Product,
		...
		// prepare condition
		$conds = array( 'img_type' => 'restaurant-branch', 'img_parent_id' => $restaurant_id );

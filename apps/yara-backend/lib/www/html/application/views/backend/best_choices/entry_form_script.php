...
function updateDataTableSelectAllCtrl(table){
	...
	function jqvalidate() {
		...
		$('#collection-form').validate({
			...
			messages:{
				name:{
					blankCheck : "<?php echo get_msg( 'err_best_choice_name' ) ;?>",
					minlength: "<?php echo get_msg( 'err_best_choice_len' ) ;?>",
					remote: "<?php echo get_msg( 'err_best_choice_exist' ) ;?>."
				},
			    image:{
	...
	function runAfterJQ() {
		...
		var table = $('#product-table').DataTable({
			 "pageLength": 20,
		      'ajax':'<?php echo site_url('/admin/products/get_all_products_for_best_choice/') .  $collection_id ?>',
		      'columnDefs': [{
		         'targets': 0,
		         'searchable':false,
		         'orderable':false,

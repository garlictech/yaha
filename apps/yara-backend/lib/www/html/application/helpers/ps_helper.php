...
if ( ! function_exists( 'deep_linking_shorten_url' ))
{
	...
	/**
	User access checking for admin app
 */


if ( ! function_exists( 'user_access_checking' )) 
{
	function user_access_checking( $url_login_user_id, $url_api_key, $user_key, $current_module )
	{

		// get ci instance
		$CI =& get_instance();

		$userkey = explode('@', $user_key);
		$form_user_id = trim($userkey[0]);
		$form_key = trim($userkey[1]);

		//print_r($form_user_id);die;
		// user checking
		$user_id = $CI->User->get_one($form_user_id)->user_id;
		$user_is_sys_admin = $CI->User->get_one($form_user_id)->user_is_sys_admin;

		// module checking
		$conds_mod['user_id'] = $user_id;
		$premissions = $CI->Permission->get_all_by($conds_mod)->result();

		$module_ids = "";

		foreach ($premissions as $per) {
	  		$module_ids .= "".$per->module_id ."" .",";
	  	}
		
		
		$modules = explode(',', $module_ids);
		
		$module_name_arr = array();
		//print_r($action);die;

		foreach ($modules as $module_id) {
			$conds_mod_id['module_id'] = $module_id;
			$module_name_arr[] = $CI->Module->get_one_by($conds_mod_id)->module_name;
		}


		if ($form_user_id != $url_login_user_id  ||  $form_key != $url_api_key) {
			// checking user_key from param and url api key and login user id
			$CI->error_response( get_msg( 'key_and_user_not_match' ));
		} else if (empty($user_id)) {
			// user_id belongs to user table or not
			$CI->error_response( get_msg( 'err_user_not_existed' ));
		} else if ($user_is_sys_admin != 1) {
			if (!in_array($current_module, $module_name_arr)) {
				// user has permission to this module or not
				$CI->error_response( get_msg( 'user_not_permission' ));
			}
		}
...
if ( ! function_exists( 'user_access_checking_action' )) 
{
	function user_access_checking_action( $url_login_user_id, $url_api_key, $user_key, $current_module, $action )
	{
		...
		// for role access checking
		$role_id = $CI->User->get_one($form_user_id)->role_id;

		$has_access_action = $CI->Role->get_allowed_accesses( $role_id );

		if ($user_is_sys_admin != 1) {
			if (!in_array($action, $has_access_action)) {
				// user has permission to do this action
				$CI->error_response( get_msg( 'user_not_action' ));
			}
		}

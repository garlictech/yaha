 ...
 * REST API for News
 */
class AdminPurchasedcategories extends Categories
{
	...
	{
		parent::__construct( 'Purchasedcategory' );
	}
	...
	// get purchase categories list

	function get_purchased_categories_get()
	{
		...
		if ( $id ) {
			...
			$data = $this->Purchasedcategory->get_one( $id );
		...
		// if limit & offset is not empty
			...
			$data = $this->Purchasedcategory->get_purchased_category_by( $conds, $limit, $offset )->result();
		} else if ( !empty( $limit )) {
		// if limit is not empty

			$data = $this->Purchasedcategory->get_purchased_category_by( $conds, $limit )->result();
		} else {
		// if both are empty

			$data = $this->Purchasedcategory->get_purchased_category_by( $conds )->result();
		}
	...
	function convert_object( &$obj )
	{
		...
		$this->ps_adapter->convert_purchased_category( $obj );
	}

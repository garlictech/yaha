 ...
 * REST API for News
 */
class AdminPurchasedfoods extends Products
{
	...
	{
		parent::__construct( 'Purchasedproduct' );
	}
	...
	// Most Purchased foods list

	function get_purchased_foods_get()
	{
		...
		// if limit & offset is not empty
			...
			$data = $this->Purchasedproduct->get_purchased_product_by( $conds, $limit, $offset )->result();
		} else if ( !empty( $limit )) {
		// if limit is not empty

			$data = $this->Purchasedproduct->get_purchased_product_by( $conds, $limit )->result();
		} else {
		// if both are empty

			$data = $this->Purchasedproduct->get_purchased_product_by( $conds )->result();
		}

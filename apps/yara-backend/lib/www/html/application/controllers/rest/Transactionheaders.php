...
class Transactionheaders extends API_Controller
{
	...
	*/
	function delivery_check_and_calculate_post()
	{
		...
		$address1 = $this->Shop->get_one($shop_id)->address1;
        ...
        if ($deliver_distance >= $distance) {
			...
			if ( $deliver_distance != '0' && $address1 != "") {
				//both are not blank
				$shop_deliver_msg = get_msg('cannot_order_food') . get_msg('shop_deliver_around') . ' ' . $deliver_distance . ' ' .$deliver_unit . ' ' . get_msg('of_label') . ' '. $address1;
				$this->error_response( $shop_deliver_msg, 503);

			}	elseif ( $deliver_distance != '0' && $address1 == "") {
				//deliver distance existed, address blank
				$shop_deliver_msg = get_msg('cannot_order_food') . get_msg('shop_deliver_around') . ' ' . $deliver_distance . ' ' .$deliver_unit;
				$this->error_response( $shop_deliver_msg, 503);

			} elseif ( $deliver_distance == '0' && $address1 != "") {
				//deliver distance blank, address existed
				$shop_deliver_msg = get_msg('cannot_order_food') . get_msg('shop_deliver_around') . ' ' . $address1;
				$this->error_response( $shop_deliver_msg, 503);

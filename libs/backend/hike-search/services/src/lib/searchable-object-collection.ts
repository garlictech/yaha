...
import { createPagedArrayLimitIndicator } from '@bit/garlictech.universal.shared.fp';
import { GraphqlApiClient } from '@bit/garlictech.universal.shared.graphql-api';
...
interface SearchParameterBase {
  resultFilter?: (place: Partial<Place>[]) => Partial<Place>[];
  maxItemNo?: number;
  from?: number;
  placeType: PlaceType;
}

interface SearchInCircleParameters extends SearchParameterBase {
  circle: Circle;
}

interface SearchImagesInCircleParameters extends SearchParameterBase {
  circle: Circle;
  placeType: PlaceType.image;
}

interface SearchByTitleParameters extends SearchParameterBase {
  searchTerm: string;
}
...
export const searchSafeImagesInCircle = ({
  ...
  maxItemNo,
  from = 0,
}: SearchImagesInCircleParameters): DependencyReader =>
  executeSearch(
    {
      lat: circle.center.lat,
      lon: circle.center.lon,

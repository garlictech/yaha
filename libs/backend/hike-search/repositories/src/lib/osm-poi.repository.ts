...
import { validateSchema } from '@bit/garlictech.universal.shared.joi-validator';
...
export class OsmPoiService {
  ...
  get(
    ...
    return this._http
      ...
      .pipe(
        buildRetryLogic<any>({ logger: Logger }),
        map((response: any) => _.get(response, 'elements', [])),
        map(response =>
          fp.filter(

...
import 'package:flutter_yaha_lib/domain/entities/shared/active_point.dart';
import 'package:flutter_yaha_lib/domain/use-cases/hike/hike.dart';

import '../../../utils/geometry/geocalc.dart';
import '../../entities/entities.dart';
import '../../entities/hike/active_hike_entity.dart';

part "active_hike.g.dart";
...
class ActiveHike extends _$ActiveHike {
  ...
  ActiveHikeEntity? build(String hikeId) {
    ...
    final List<ActivePoint> activePoints = range
        .map(
            (i) => ActivePoint(point: coordinates[i + 1], arrival: arrivals[i]))
        .toList();

    activePoints.insert(
        0, ActivePoint(point: coordinates[0], arrival: settings.startTime));

    return ActiveHikeEntity(activePoints: activePoints);
  }
}
